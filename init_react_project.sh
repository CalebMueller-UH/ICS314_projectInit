...
add_template_files() {
  ...
  cp $TEMPLATE_FILES_DIR/README.md ./$DEST_DIR

  cp -r $TEMPLATE_FILES_DIR/my-app ./$DEST_DIR
...
init_local_repo() {
  ...
  # cd my-app
  # npm install react-bootstrap bootstrap react-bootstrap-icons --save
  # rm -f src/*
  ...
  read -p "What would you like to use as the title for this react page?" TITLE
  if [[ -n $TITLE ]]; then
    TITLE = $DEST_DIR
  fi

  # Modify the public/index.html page title to TITLE
  sed -i "s|<title>.*</title>|<title>$TITLE</title>|" "./$DEST_DIR/my-app/public/index.html"
...
# Replace IslandSnow with name of project
sed -i "s/IslandSnow/%DestDir/g" $DEST_DIR/*

# spawn console windows to run the react app and intelliJ IDEA
gnome-terminal --working-directory="$DEST_DIR" -- npm start
gnome-terminal --working-directory="$DEST_DIR" -- idea.sh .
